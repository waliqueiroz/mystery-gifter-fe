...
4. **Dado** um usuário no detalhe de um grupo, **Quando** ele convida um membro ou edita algum dado (fluxos com formulário ou múltiplos passos), **Então** essas ações abrem em **rotas dedicadas**; **Quando** ele apenas visualiza rapidamente os detalhes de um membro (leitura), **Então** o conteúdo aparece em **bottom sheet deslizante**; em ambos os casos os comportamentos funcionais permanecem idênticos aos atuais e o visual segue o DESIGN.md.
...
- **FR-006**: Todos os tokens visuais (cores, raios, espaçamentos, tipografia, sombras) DEVEM ser declarados como custom properties em um único arquivo de tema central, e nenhum valor de cor (hex/rgb/hsl), raio, sombra ou tamanho tipográfico DEVE aparecer hardcoded fora desse arquivo.
- **FR-007**: Classes utilitárias específicas do projeto DEVEM ser prefixadas com `mg-` (ex.: `mg-pill`, `mg-app-nav`) para evitar colisão com nomes genéricos.
...
- **SC-012**: 0 (zero) modais não-confirmatórios remanescem no produto após o merge; todo fluxo de criação, edição ou exibição de detalhes que hoje usa modal está migrado para rota dedicada, página em tela cheia ou painel deslizante app-like.
